...
def test_sanitize_for_c_comment_replaces_apostrophes():
    """Python-repr witness values containing raw apostrophes (e.g.,
    ``{'name': 'unknown'}``) embedded inside a ``/* ... */`` comment
    triggered ``warning: missing terminating ' character`` →
    CONVERSION ERROR → exit 6 on CBMC parse. Replace ``'`` with a safe
    substitute."""
    from bmc_agent.harness_generator import _sanitize_for_c_comment
    out = _sanitize_for_c_comment("{'name': 'unknown'}")
    assert "'" not in out
    # Replacement is `backtick` (visually similar, no special meaning in C)
    assert "`" in out


def test_sanitize_for_c_comment_replaces_double_quotes():
    """Double-quotes inside a comment are also a tokenization hazard
    (un-terminated string-literal warnings)."""
    from bmc_agent.harness_generator import _sanitize_for_c_comment
    out = _sanitize_for_c_comment('foo "bar" baz')
    assert '"' not in out


def test_sanitize_for_c_comment_escapes_comment_terminator():
    """``*/`` inside an embedded value would prematurely close the
    enclosing comment, leaving the rest as code → parse error.
    Sanitize to ``*\\/``."""
    from bmc_agent.harness_generator import _sanitize_for_c_comment
    out = _sanitize_for_c_comment("size_t /* 120ul */ extra")
    assert "*/" not in out


def test_sanitize_for_c_comment_truncates_long_values():
    """Comment lines bounded so a huge witness dump doesn't push
    column positions where downstream tools choke."""
    from bmc_agent.harness_generator import _sanitize_for_c_comment
    src = "a" * 5000
    out = _sanitize_for_c_comment(src, max_len=200)
    assert len(out) <= 210  # 200 + "..." + slack
    assert out.endswith("...")


def test_sanitize_for_c_comment_empty_passes():
    """Empty / None inputs return empty string, don't crash."""
    from bmc_agent.harness_generator import _sanitize_for_c_comment
    assert _sanitize_for_c_comment("") == ""


def test_reachability_harness_header_comment_sanitized(tmp_path):
    """End-to-end: a CEx with Python-repr witness values produces a
    reachability harness whose header comment does NOT contain raw
    apostrophes — verifies the sanitizer is wired into the harness
    emission, not just sitting unused."""
    from bmc_agent.parser import (
        FunctionInfo, FunctionSignature, ParsedCFile,
    )
    from bmc_agent.harness_generator import HarnessGenerator
    ...
    callee_sig = FunctionSignature(
        name="callee", return_type="void",
        parameters=[("void", "")],
    )
    parsed = ParsedCFile(
        path=str(tmp_path / "fake.c"),
        functions={"caller": caller_sig, "callee": callee_sig},
        function_bodies={"caller": caller.body},
        call_graph={"caller": {"callee"}, "callee": set()},
    )
    cex = Counterexample(
        failing_property="callee.assertion.1",
        # Python-repr style witness with raw apostrophes — the regression case
        variable_assignments={
    ...
    text = gen.generate_reachability_harness(
        caller=caller, callee_name="callee",
        counterexample=cex, caller_spec=spec,
        parsed_file=parsed,
    )

    # Header comment must not contain raw apostrophes anywhere in its body
    # — the entire dict-repr was previously dumped raw between /* and */
    header = text.split("/* Generated by AMC Phase 3")[0]
