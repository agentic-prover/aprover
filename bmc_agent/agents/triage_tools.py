...
_TOOLS_PROMPT_ADDENDUM = (
    ...
    "on LIKELY_FP votes.\n\n"
    "OUTPUT — this OVERRIDES every 'vote REAL_BUG / vote LIKELY_FP'\n"
    "phrasing above. Do NOT emit a bare {\"verdict\": ...}; emit the\n"
    "STRUCTURED JSON from the base instructions (witness_reproducible_as_is,\n"
    "real_defect, defect_site, defect_reachable_in_tree, needs_human,\n"
    "confidence, reasoning). Translate the audit into those fields:\n"
    "  * An UNBUDGETED write reachable from a public caller -> real_defect=true,\n"
    "    defect_reachable_in_tree=true (the REAL_BUG case).\n"
    "  * A reachability gate (G1/G4/G5) firing means the CEx precondition is NOT\n"
    "    reachable from any in-tree caller -> set defect_reachable_in_tree=false.\n"
    "    A FIRED GATE IS NOT AUTOMATICALLY LIKELY_FP. Decide real_defect\n"
    "    INDEPENDENTLY: is the UNDEFENDED condition a genuine defect a future /\n"
    "    adversarial / contract-violating caller WOULD trigger (e.g. calloc not\n"
    "    checking nmemb*size overflow; a public API missing a bounds check)? If\n"
    "    YES -> real_defect=true, and the DERIVED verdict is LATENT (a real defect\n"
    "    no in-tree caller reaches yet). Set real_defect=false ONLY when the code\n"
    "    is correct under its REAL contract — the 'missing' precondition is a\n"
    "    legitimate universal invariant every valid caller must honor (like the\n"
    "    next_field *l<=strlen(*p) helper contract), not a bug (derived LIKELY_FP).\n"
    "  * witness_reproducible_as_is=false when a stub/harness was more permissive\n"
    "    than the real callee (stub-disconnect); a genuine underlying defect still\n"
