...
def generate_reports(sweep_output: str | Path, driver: str, rerun_cmd: str | None = None) -> list[str]:
    ...
    if rows:
        ...
        index_lines += [
            ...
            f"| `observed_safety_concern` | {by_dyn.get('observed_safety_concern', 0)} | Target replay reached an unsafe operation, but the target did not expose a crash/panic marker. Count as a safety concern, not a QEMU-confirmed crash. |",
        ...
        index_lines += [
            ...
            "2. The **dynamic outcome** column tells you whether runtime/target replay added evidence. `confirmed` is the strongest evidence; `observed_safety_concern` means the target reached an unsafe operation without a target-visible fault; `not_triggered` is the most ambiguous (could be a real bug with a different attacker input, could be an FP).",
