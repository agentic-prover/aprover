...
CBMC reports a `malloc.overflow.1` failure — a arithmetic / overflow violation in `malloc`.

**Validator reasoning:** Counterexample state is reachable from caller(s): ['calloc']. Call chain: ['calloc', 'malloc']. Full chain traced to system entry.

## How to trigger

Reach `malloc` via the call chain `calloc → malloc` and supply inputs that match the counterexample variable assignments above.
