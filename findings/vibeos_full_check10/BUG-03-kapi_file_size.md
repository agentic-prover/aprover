...
CBMC reports a `main.assertion.2` failure — a semantic / contract violation in `kapi_file_size`.

**Realism checker's key concern:** The function checks for NULL but not for pointer validity. Any non-null invalid pointer bypasses the guard and causes an uncontrolled memory read at `n->size`, which is directly exploitable from external callers of this API.

**Validator reasoning:** 'kapi_file_size' is an entry function (no callers in any file). The counterexample is directly reachable from the system boundary.

## How to trigger

`kapi_file_size` is reachable as a system-entry point — call it directly with the counterexample's variable assignments.
